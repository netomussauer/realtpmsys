...
- [x] **Mirror push reverso Gitea → GitHub** (resolve webhook automático sem
  polling — causa raiz confirmada em 2026-09-01, corrigido em 2026-09-04):
  o mirror Gitea (`labadmin/realtpmsys.git`) só fazia *pull* periódico do
  GitHub, e isso não passa pelo mesmo code path de um `git push` real —
  não disparava o webhook do Gitea de jeito nenhum, por mais rápido que o
  polling fosse. Confirmado na prática antes da correção: um POST
  sintético do formato exato que o Gitea manda, enviado direto pro
  EventListener (`el-realtpmsys-event-listener`), criou um PipelineRun
  genuíno (label `trigger: gitea-push`, não `manual`) que buildou e
  publicou a imagem com sucesso — a cadeia
  EventListener→Trigger→PipelineRun funciona de ponta a ponta. No
  caminho, achado e corrigido um bug real: `serviceAccountName` no
  `TriggerTemplate` estava no campo raiz de `PipelineRun.spec`
  (deprecado, ignorado pelo Tekton Pipelines v1) em vez de
  `taskRunTemplate.serviceAccountName` — a task final `rollout-restart`
  rodava como `system:serviceaccount:cicd:default` (sem a Role
  `tekton-rollout` cross-namespace) e falhava com 403, mesmo com
  build+push funcionando. Corrigido em `infra/tekton/triggers.yaml`.
  Resolvido de fato invertendo a direção do mirror: o repo Gitea foi
  convertido de mirror (pull) para repositório regular com um *push
  mirror* configurado pro GitHub — agora o Gitea é o remoto primário
  (`origin`), GitHub é sincronizado automaticamente a cada push, e o
  webhook do Gitea dispara nativamente em cada push real, sem depender
  de polling. Nenhuma mudança de infra foi necessária: o
  `TriggerTemplate` já clonava do Service DNS interno do Gitea
  independente da direção do mirror.
