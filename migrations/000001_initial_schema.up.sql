...
-- Gerado a partir de docs/schema.sql
-- Aplicar com: make migrate-up
...
CREATE UNIQUE INDEX uq_responsavel_principal_por_atleta
    ON responsaveis (atleta_id) WHERE contato_principal = TRUE;
...
CREATE TABLE uniformes (
    ...
    tam_chuteira  VARCHAR(5)  NOT NULL,
    atualizado_em TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE turmas (
    ...
    CONSTRAINT chk_turma_faixa CHECK (
        faixa_etaria_min >= 4 AND faixa_etaria_max <= 18 AND faixa_etaria_min <= faixa_etaria_max
    ),
...
CREATE UNIQUE INDEX uq_matricula_ativa_por_turma
    ON matriculas (atleta_id, turma_id) WHERE status = 'ATIVA';
...
CREATE UNIQUE INDEX uq_contrato_ativo_por_atleta
    ON contratos (atleta_id) WHERE status = 'ATIVO';
...
CREATE TRIGGER trg_contratos_updated    BEFORE UPDATE ON contratos    FOR EACH ROW EXECUTE FUNCTION fn_atualizar_timestamp();
...
VALUES ('admin@realtpmsys.local', '$2a$12$placeholder_trocar_no_primeiro_login', 'ADMIN');
